...
static int wemac_phy_init(wemac_board_info_t *db)
{
    ...
    if (db->mos_gpio){
        db->mos_gpio->data = 1;
        gpio_set_one_pin_status(db->mos_pin_handler,db->mos_gpio,"power",1);
...
//	int i = 0;
    ...
    if (db->mos_gpio){
        db->mos_gpio->data = 0;
		gpio_set_one_pin_status(db->mos_pin_handler,db->mos_gpio,"power",0);
      //  __gpio_set_value(db->mos_gpio->gpio.gpio, db->mos_gpio->gpio.data);
	...
	if(db->gpio_vbase){
		...
		struct resource iomem = platform_get_resource(dev, IORESOURCE_MEM, 2);
		release_mem_region(iomem->start, resource_size(iomem));
	}
#else
...
/*#ifdef 0// CONFIG_NET_POLL_CONTROLLER
...
static void gmac_generate_mac_by_chip_id(uint8_t *addr)
{
	...
	uint8_t chipid[17];//={0x6E,0x3D,0xEF,0xDB,0xC5,0x7E,0x32,0x6E,0x3D,0xEF,0xDB,0xC5,0x7E,0x32,0xaa,0xaa,0xaa};
