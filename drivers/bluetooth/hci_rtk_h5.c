...
#define H5_BTSTATE_CHECK_CONFIG_PATH      "/lib/firmware/rtlbt/need_check_bt_state"
...
static int h5_config_state_check(void) {
	...
	int result=0;
	set_fs (KERNEL_DS);
    printk("enter %s config_path=%s\r\n",__FUNCTION__,config_path);
	//open file
	filp = filp_open(config_path, O_RDONLY, 0);
	if (IS_ERR(filp)) {
	     printk("open h5 bt state check config file fail?, chmod?\n");
	     result=-1;
	   goto error2;
	  }

	if(!(filp->f_op) || !(filp->f_op->read) ) {
	     printk("file %s cann't readable?\n",config_path);
	     result = -1;
	     goto error1;
	}
	...
	if(memcmp(buffer,"0",1)==0) {
		...
		printk("close check bt state");
	}
	if(memcmp(buffer,"1",1)==0) {
		//check bt state on
		need_check_bt_state = 1;
		printk("open check bt state");
	}
